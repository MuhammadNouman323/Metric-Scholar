<x-admin>
    <div class="p-6 md:p-10 lg:p-12 pb-24 max-w-[1400px] mx-auto min-h-screen">
        ...
        <div class="max-w-4xl mx-auto space-y-8 mt-8">
            <div class="space-y-2">
                <h1 class="text-[40px] font-extrabold text-[#1f2937] leading-tight tracking-tight">Edit Course</h1>
                <p class="text-gray-500 text-[15px] font-medium">Update the details for {{ $course->title }}.</p>
            </div>

            <div class="bg-white border border-gray-100 rounded-[1.5rem] overflow-hidden shadow-sm">
                <div class="h-2 bg-[#0e48c1]"></div>

                <form action="{{ route('admin.departments.courses.update', [$department, $course]) }}" method="POST" class="p-7 md:p-9 space-y-8">
                    @csrf
                    @method('PUT')
                    ...
                    <section class="space-y-5">
                        ...
                        <div class="space-y-2">
                            ...
                            <input id="title" name="title" type="text" placeholder="e.g. Introduction to Computer Science" value="{{ old('title', $course->title) }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <input id="code" name="code" type="text" placeholder="e.g. CS-101" value="{{ old('code', $course->code) }}"
                                class="w-full border border-gray-100 bg-[#f8fafc] rounded-xl px-4 py-3 text-[14px] text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0e48c1] focus:bg-white" />
                            @error('code')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <select id="semester" name="semester"
                                    ...
                                    <option value="Spring 2024" {{ old('semester', $course->semester) === 'Spring 2024' ? 'selected' : '' }}>Spring 2024</option>
                                    <option value="Fall 2024" {{ old('semester', $course->semester) === 'Fall 2024' ? 'selected' : '' }}>Fall 2024</option>
                                </select>
                                @error('semester')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="space-y-2">
                                ...
                                <input id="credit_hours" name="credit_hours" type="number" min="1" max="8"
                                    placeholder="3" value="{{ old('credit_hours', $course->credit_hours) }}"
                                    class="w-full border border-gray-100 bg-[#f8fafc] rounded-xl px-4 py-3 text-[14px] text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0e48c1] focus:bg-white" />
                                @error('credit_hours')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
